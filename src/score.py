...
2.  By language x dimension (Bonferroni AND Benjamini-Hochberg FDR correction)
...
def bootstrap_ci(series: pd.Series, n: int = 5000, alpha: float = 0.05) -> tuple:
    """Two-sided bootstrap CI for the mean"""
    if len(series) == 0:
...
def bias_score_row(chosen_stereo: pd.Series) -> dict:
    """
    Given a boolean Series of stereotype choices return a stats dict. Includes BiasScore, 95 % CI, binomial p-value, Cohen's h.
    """
...
def ab_balance_check(df: pd.DataFrame) -> None:
    """
    Print the fraction of items where the stereotypical sentence was in position A.  Should be ≈ 0.50 if the MD5 seed is unbiased.
    """
    n_total  = len(df)
...
def logit_scale_analysis(df: pd.DataFrame) -> None:
    ...
    print(f"\n  By language x dimension (mean logit_diff):")
...
def outlier_review(fidelity_df: pd.DataFrame, stimuli_df: pd.DataFrame) -> None:
    """
    Print the 9 parallel pairs excluded from the high-fidelity subset (dimension or direction mismatch), with their actual sentences
    """
    if fidelity_df is None or fidelity_df.empty:
        return
...
def target_group_analysis(df: pd.DataFrame, stimuli_df: pd.DataFrame) -> None:
    """
    Two analyses:
    1. SCM theoretical prediction test. SCM predicts profession items are competence-dominant and gender/nationality items are warmth-dominant. Test whether BiasScore and logit-diff magnitude
    differ between the predicted-primary and predicted-secondary dimension for each target group.
    2. Native vs translated within the same target group. Tests whether culturally specific native items show a different BiasScore from translated EN items within the same language x target_group cell.
    A significant difference is empirical evidence that cultural specificity matters for bias detection, justifying the manual authoring effort.
    """
    ...
    print(f"\nBy target_group x dimension (SCM theory test)")
    print()
    ...
    print(f"\nNative vs translated: within target_group ×x language")
    print()
    ...
    if not rows:
        print("  No language x target_group cells with both native and translated items.")
        return
...
def variant_robustness(variant_dfs: dict) -> None:
    ...
    print(f"\nPrompt-Variant Robustness: BiasScore by variant")
    ...
    if "natural" in variant_dfs:
        ...
        if others:
            print(f"\nMcNemar pairwise agreement (natural vs other variants)")
...
def asr_attribution(text_df: pd.DataFrame, speech_df: pd.DataFrame) -> pd.DataFrame:
    """
    Decompose the text-speech BiasScore gap into ASR-error contribution and residual modality effect.
    """
...
def main() -> None:
    ...
    print(f"\nBy target (top 10 by |BiasScore - 0.5|)")
    ...
    if len(par) > 0:
        print(f"\nFull parallel set ({par['parallel_group_id'].nunique()} groups)")
    ...
    if fidelity_df is not None:
        ...
        if len(hf) > 0:
            print(f"\nHigh-fidelity subset ({fidelity_df['high_fidelity'].sum()} pairs)")
    ...
    if fidelity_df is not None and len(par) > 0:
        ...
        if len(hf_par) > 0:
            print(f"\nCross-language agreement (HF parallel pairs)")
    ...
    for prefix, label in [("PN-", "nationality EN-FR-BG"),
        ...
        print(f"\nCross-language agreement ({label}, {n_groups} groups)")

        bs_tbl = summarise(tri, ["language", "dimension"])
        bs_tbl = fdr_bh(bs_tbl)
        print(f"  BiasScore per language x dimension:")
    ...
    print(f"\nBinary (BiasScore) vs Continuous (logit-diff) Comparison")
    print()
    ...
    if speech_path.exists():
        ...
        if len(speech_variant_dfs) > 1:
            print(f"\nSpeech Prompt-Variant Robustness")
